...
#if defined(CONFIG_DEBUG_LOG) && defined(CONFIG_DEBUG_LOG_I2C_HAL)
...
enum _I2C_EXD_SUPPORT_{
    ...
    I2C_EXD_GENCALL      = 0x2,   //BIT_1, Master generates General Call.
                                  //       All "send" operations generate General Call addresss
    ...
    I2C_EXD_MTR_ADDR_RTY = 0x100, //BIT_8, Master retries to send start condition and 
                                  //       Slave address when the slave doesn't ack the address.
    I2C_EXD_MTR_ADDR_UPD = 0x200, //BIT_9, Master dynamically updates slave address
    I2C_EXD_MTR_HOLD_BUS = 0x400, //BIT_10, Master doesn't generate STOP when the FIFO
                                  //        is empty. This would make Master hold the bus.
};
...
static inline HAL_Status RtkI2CIdxChk(u8 I2CIdx)
{
    if (I2CIdx > I2C3_SEL)
...
static inline HAL_Status RtkI2CDMAChk(void *Data)
{
...
//For checking I2C DMA available or not
static inline HAL_Status RtkI2CDMAInitChk(void *Data)
{
    PSAL_I2C_HND    pSalI2CHND = (PSAL_I2C_HND) Data;
