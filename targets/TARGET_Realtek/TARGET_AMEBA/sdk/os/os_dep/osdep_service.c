...
void add_mem_usage(_list *pmem_table, void *ptr, int size, int *used_num, int flag)
{
    ...
    if (ptr == NULL) {
        DBG_ERR("Catch a mem alloc fail with size of %d, current heap free size = %d",
		size, free_heap_size);
        return;
    } else {
...
int rtw_create_worker_thread(rtw_worker_thread_t* worker_thread, u8 priority,
                             u32 stack_size, u32 event_queue_size)
{
    ...
    if (rtw_init_xqueue(&worker_thread->event_queue, "worker queue",
                         sizeof(rtw_event_message_t), event_queue_size) != SUCCESS) {
        return FAIL;
    }

    if (!rtw_create_task(&worker_thread->thread, "worker thread",
