...
static int _rtx2_init_xqueue(_xqueue* queue, const char *name, u32 msg_size, u32 msg_num)
{
_func_enter_;
    ...
    mbox->queue_mem = _rtx2_zmalloc(msg_num * (msg_size + sizeof(os_message_t)));
    if (mbox->queue_mem == NULL) {
        goto err_exit;
    }
    *queue = (_xqueue)mbox;
    mbox->attr.mq_mem = mbox->queue_mem;
    mbox->attr.mq_size = msg_num * (msg_size + sizeof(os_message_t));
    mbox->attr.cb_mem = &mbox->data;
    mbox->attr.cb_size = sizeof(mbox->data);
    mbox->id = osMessageQueueNew(msg_num, msg_size, &mbox->attr);
    if (mbox->id == NULL) {
        goto err_exit;
    }
...
err_exit:
    ...
    if (mbox) {
        if (mbox->queue_mem) {
            _rtx2_mfree(mbox->queue_mem, msg_num * (msg_size + sizeof(os_message_t)));
        }
...
u32 _rtx2_timerDelete(_timerHandle xTimer, osdepTickType xBlockTime)
{
_func_enter_;
...
u32 _rtx2_timerStop(_timerHandle xTimer, osdepTickType xBlockTime)
{
_func_enter_;
    rtx_tmr_t *tmr = (rtx_tmr_t *) xTimer;
